...
class comm_detect(object):
    ...
    def infomap(self):
        ...
        """
        cur = self.con.cursor()
        with open('network_update.pickle', 'rb')  as f:
        ...
        with open('communities_updated.pickle', 'wb') as f:
    ...
    def create_comm_db(self):
        ...
        """
        with open('communities_updated.pickle', 'rb') as f:
            communities = pickle.load(f)
        with open('network_update.pickle', 'rb') as f:
            network = pickle.load(f)

        query = 'SELECT * FROM node_update;'
        db =  pd.read_sql_query(query, con)

        query2 = 'SELECT node, song_uri, popularity FROM classical_update;'
        ...
        community_db.to_sql('node_communities_update', engine, if_exists='replace', index = False)
